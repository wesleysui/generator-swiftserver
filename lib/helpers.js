 ...
 */
exports.validateFilePathOrURL = function (path) {
  if (!path) {
    return 'Path is required'
  }

  // ensure file actually exists if using local file path
  var localMatch = /^\w+|^\/|^\.\.?\//.test(path)
  var remoteMatch = /^https?:\/\/\S+/.test(path)
  if (localMatch && remoteMatch) {
    return true
  } else if (localMatch && !remoteMatch) {
    return fs.existsSync(path) ? true : format('The following path does not exist: %s', path)
  }
  return format('The following is not a valid path: %s', path)
}
...
function getCloudServiceConfig (serviceType, service) {
  switch (serviceType) {
    ...
    case 'redis':
      ...
      var uri = credentials.uri || `redis://${credentials.username || 'admin'}:${credentials.password || ''}@${credentials.host || 'localhost'}:${credentials.port || '6397'}`
      return {
        name: service.name,
        label: service.label || exports.getBluemixServiceLabel(serviceType),
        tags: [],
    ...
    case 'appid':
      return {
        ...
        credentials: {
          ...
          version: service.credentials.version || 3
        }
      }
