...
exports.performSDKGenerationAsync = function (sdkName, sdkType, fileContent) {
  ...
  return request.postAsync({
    ...
    .tap(function (response) {
      ...
      if (!parsedResponse.job || (parsedResponse.job && !parsedResponse.job.id)) {
        throw new Error(chalk.red('SDK generator recieved unrecognised response body: ', response.body))
      }
    })
...
exports.getClientSDKAsync = function (sdkName, generatedID) {
  ...
  return new Promise((resolve, reject) => {
    ...
    .on('error', err => {
      throw new Error(chalk.red('Getting client SDK failed with error: ', err))
    })
...
exports.getServerSDKAsync = function (sdkName, generatedID) {
  ...
  debug(`starting server SDK download and unzip for ${sdkName} from ${serverDownloadURL}`)
  // Use the non-async version of request.get() here because
  // we are going to use .pipe() to stream the data to disk
  return new Promise((resolve, reject) => {
    ...
    .on('error', err => {
      throw new Error(chalk.red('Getting server SDK failed with error: ', err))
    })
